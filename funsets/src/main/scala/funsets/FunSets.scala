...
trait FunSets extends FunSetsInterface {
  ...
  def forall(s: FunSet, p: Int => Boolean): Boolean = {
    def iter(a: Int): Boolean = {
      // println(s"a: $a, contains(s, a) && p(a): ${ contains(s, a) && p(a) }")
      if (a > bound) true
   ...
   */
  def map(s: FunSet, f: Int => Int): FunSet = x => exists(s, p => f(p) == x)
  // def map(s: FunSet, f: Int => Int): FunSet = (x: Int) => exists(s, (p: Int) => s(f(x)) )
